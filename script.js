...
(function () {
    ...
    // ── escaping fighter ghosts (DOM canvas that float upward) ──
    const escapingFighters = [];

    function spawnEscapingFighter(fighter) {
        const rect = cv.getBoundingClientRect();
        // world position of the fighter's feet in viewport coords
        const vpX = rect.left + fighter.x;
        const vpY = rect.top + fighter.y;   // feet
        ...
        escapingFighters.push({
            el: ghost,
            x: vpX,            // viewport X (center)
            y: vpY - 80,       // viewport Y (top of canvas el)
            vx: (Math.random() - 0.5) * 3,
            vy: -8 - Math.random() * 5,  // initial upward burst
            rot: 0,
            rotV: (Math.random() - 0.5) * 6,
            gy: 0.18,           // gentle gravity — floats more than falls
            a: 1,
            dir: fighter.dir,
            tick: 0,
            state: fighter.state,
    ...
    function updateEscapingFighters() {
        for (let i = escapingFighters.length - 1; i >= 0; i--) {
            ...
            g.vy += g.gy;
            g.rot += g.rotV;
            // fade out as it rises (starts fading after leaving viewport top)
            ...
            // redraw ghost onto its own canvas
    ...
    // track which fighters have already spawned a ghost this air-time
    // so we only fire once per jump-above-canvas
